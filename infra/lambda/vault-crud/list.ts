...
export async function listEntries(
  ...
  if (!parsed.success) {
    throw ApiError.invalidInput(parsed.error.issues[0]?.message ?? "Invalid query");
  }

  const { project_id, scope, tags, page, limit } = parsed.data;

  if (scope === "project" && !project_id) {
    throw ApiError.invalidInput("scope=project requires a project_id query parameter");
  }
  ...
  type Row = Record<string, unknown> & { total: string };
  const rows = await query<Row>(sql, params);
  const total = rows.length > 0 ? Number(rows[0].total) : 0;
  const entries = rows.map(({ total: _t, ...entry }) => entry);
