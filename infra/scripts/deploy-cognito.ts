...
// Reuses the same config files as deploy.ts — only ProjectPrefix, EnvironmentName,
// region, and tags are needed for the Cognito stack.
type BaseConfig = {
...
function stackName(config: BaseConfig): string {
  return `${config.project_prefix}-cognito-${config.environment}`;
}
